...
import { Atom, HelpCircle, Mail, ShieldAlert, Heart } from 'lucide-react';
...
export default function Footer({ identity, setCurrentTab }: FooterProps) {
    ...
    <footer className="transition-all duration-300 border-t border-white/10 bg-[#070708] text-zinc-300 relative z-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              {isTurtle ? (
                ...
                <div className="p-1.5 rounded-lg bg-white/5 border border-white/10 text-blue-400 shrink-0">
                  <Atom className="w-4 h-4" />
                </div>
              )}
              <span className="font-display font-bold text-base tracking-tight text-white">
                {isTurtle ? 'Turtle Rock Science Club' : 'Kinetic Lab'}
              </span>
            </div>
            <p className="text-xs text-zinc-400 leading-relaxed font-sans">
              {isTurtle
                ? 'Nurturing the next generation of naturalists and innovators through community-centered, hands-on scientific adventure.'
                : 'Accelerating rigorous scientific inquiry and methodology through structured experimentation and engineering design.'}
            </p>
          </div>

          {/* Site Navigation Links */}
          <div>
            <h4 className="font-mono font-bold text-xs uppercase tracking-widest mb-4 text-zinc-500">Explore Labs</h4>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5 text-xs font-mono text-zinc-300">
              <p className="flex items-center gap-1.5">
                ...
                hello@{isTurtle ? 'turtlerockscience.org' : 'kineticlab.dev'}
              </p>
        ...
        <div className="mt-12 pt-6 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4 text-[11px] text-zinc-500">
          <p>© 2026 {isTurtle ? 'Turtle Rock Science Club' : 'Kinetic Lab Inc'}. All rights reserved.</p>
