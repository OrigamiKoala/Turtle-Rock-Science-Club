...
Two pages are plain, self-contained HTML (no React, no build step, no external
assets) that live outside the SPA. GitHub Pages serves them at
`/ariadne-console/` and `/station-1c/` (it redirects the no-slash form):

- `public/ariadne-console/index.html` — the Ariadne ship-systems terminal from
  the chem-lessons curriculum (`~/chem-lessons/ariadne-console.html`, which is
  the source of truth; copy it over by hand when it changes). Supports
  `?week=N` and `#1,2,3` exactly as the original does.
- `public/station-1c/index.html` — a digital version of Session 1's Station
  1-C "The Beam Deflector" (`~/chem-lessons/session-01-reactor.md` § 1-C),
  in the console's visual style. It has a simulated ramp and magnet with a
  distance slider (replacing the physical ramp and ball bearings; magnet
  strength `K` was tuned with a Node script so the ball sticks up close,
  curves hard at middle distances and bends only slightly when far) and an
  animated sensor trace of the three beams passing the + plate. It
  deliberately never reveals or checks the answer (beam 1): pods write it in
  their paper logbook, same as the printed card. All three beams are drawn
  the same colour so colour can't give the answer away. Keys: `F`
  fullscreen, `R` reset for the next pod.

Nothing in the React app links to either page.
...
workshops, a "Science Tidbits" section exploring cool facts on next week's workshop topics, and a weekly "In the News" science digest section). `01-lab-log-entry.html` is the web/responsive HTML version tailored for the Lab Log tab in Google Sheets (retains all Issue 1 sections, formatted cleanly without email boilerplate to fit comfortably under cell and JSON payload limits). `02-cs-chemistry.html` is the Issue 2 newsletter for the week leading up to 9/26 (features custom cartoon digital ink artwork combining Chemistry and Computer Science, dedicated Upcoming Events cards for the 9/26 Chemistry Session #2 and Computer Science Session #1 workshops, kid-friendly Science Tidbits exploring lava lamp internet security and molecular chirality, In the News breakthroughs on a giant new Moon crater, a newborn baby planet, and the Roman Space Telescope, two placeholder slots for science opportunity advertisements, the Fall Workshop Schedule highlighting 9/26, Master Doc reference card, and condensed community contacts). `02-lab-log-entry.html` is the web/responsive HTML version of Issue 2 tailored for the Lab Log tab in Google Sheets (retains all Issue 2 sections, upcoming events, tidbits, news, opportunities, and schedule, formatted cleanly without email boilerplate). `confirm-subscription.html` is the double opt-in confirmation email. Note: `docs/` is gitignored.
...
- **Newsletter 02 Post Code**:
  - Saved in [docs/newsletter/02-lab-log-entry.html](file:///Users/carlliu/Turtle-Rock-Science-Club/docs/newsletter/02-lab-log-entry.html) for pasting into Google Sheets cell E3.
